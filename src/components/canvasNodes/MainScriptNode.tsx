...
export const MainScriptNode = React.memo(function MainScriptNode({ data, id, selected }: NodeProps) {
  ...
  const getVideoRatios = (modelId: string) => {
    if (modelId === "grok-video-3") {
      return ["16:9", "9:16", "1:1", "2:3", "3:2"];
    }
    return ['1:1', '16:9', '9:16', '4:3', '3:4', '21:9'];
  };

  const getVideoResolutions = (modelId: string) => {
    if (modelId === "grok-video-3") {
      return ["720p"];
    }
  ...
  const getVideoDurations = (modelId: string) => {
    if (modelId === "grok-video-3") {
      return ["6s", "10s", "15s"];
    }
    return ['5s', '10s', '15s'];
  ...
  const currentRatios = getRatios(extractParams.modelId);
  const currentResolutions = getResolutions(extractParams.modelId);
  
  const currentVideoRatios = getVideoRatios(promptParams.modelId);
  const currentVideoResolutions = getVideoResolutions(promptParams.modelId);
  const currentVideoDurations = getVideoDurations(promptParams.modelId);
        ...
        <div className="flex flex-col gap-4 relative z-10">
          ...
          {mode === 'extract' && (
            <div className="flex items-center gap-4 bg-neutral-50 px-4 py-3 rounded-xl border border-neutral-100">
                  ...
                  <div className="absolute top-full left-0 mt-1 w-[180px] bg-white border border-neutral-200 rounded-xl shadow-xl z-50 overflow-hidden py-1" onPointerDown={(e) => e.stopPropagation()}>
                    {IMAGE_MODELS.map(m => (
                      <button
                        key={m.id}
                        onClick={() => {
                          const newRatios = getRatios(m.id);
                          const newResolutions = getResolutions(m.id);
                          let newRatio = extractParams.ratio || newRatios[0];
                          if (!newRatios.includes(newRatio)) newRatio = newRatios[0];
                          let newRes = extractParams.resolution || newResolutions[0];
                          if (!newResolutions.includes(newRes)) newRes = newResolutions[0];
                          
                          updateExtractParams({
                            modelId: m.id,
                            ratio: newRatio,
                            resolution: newRes,
                            ...(m.id !== "gpt-image-2-all" && { quality: undefined })
                          });
                          setImageModelOpen(false);
                        }}
                        className={cn(
              ...
              {extractParams.modelId === "gpt-image-2-all" && (
                <div className="relative">
                  <button
                    onPointerDown={(e) => { e.stopPropagation(); setImageQualityOpen(!imageQualityOpen); }}
                    className={cn("flex items-center gap-1.5 text-xs text-neutral-700 bg-white border border-neutral-200 hover:bg-neutral-50 px-3 py-1.5 rounded-lg cursor-pointer font-medium", selected && "nodrag")}
                  >
                    <span>{
                      extractParams.quality === 'low' ? '低画质' : 
                      extractParams.quality === 'medium' ? '中等画质' : 
                      extractParams.quality === 'high' ? '高画质' : '自动画质'
                    }</span>
                    <ChevronDown className="w-3.5 h-3.5 text-neutral-400" />
                  </button>
                  {imageQualityOpen && (
                    <div className="absolute top-full left-0 mt-1 w-[100px] bg-white border border-neutral-200 rounded-xl shadow-xl z-50 overflow-hidden py-1" onPointerDown={(e) => e.stopPropagation()}>
                      {[
                        { id: 'auto', label: '自动画质' },
                        { id: 'low', label: '低画质' },
                        { id: 'medium', label: '中等画质' },
                        { id: 'high', label: '高画质' },
                      ].map(q => (
                        <button
                          key={q.id}
                          onClick={() => { updateExtractParams({ quality: q.id }); setImageQualityOpen(false); }}
                          className={cn(
                            "w-full text-left px-4 py-2 text-xs border-none bg-transparent cursor-pointer transition-colors nodrag",
                            (extractParams.quality || 'auto') === q.id ? "text-blue-600 bg-blue-50 font-bold" : "text-neutral-600 hover:bg-neutral-50"
                          )}
                        >
                          {q.label}
                        </button>
                  ...
                  <div className="absolute top-full left-0 mt-1 w-[180px] bg-white border border-neutral-200 rounded-xl shadow-xl z-50 overflow-hidden py-1" onPointerDown={(e) => e.stopPropagation()}>
                    {VIDEO_MODELS.map(m => (
                      <button
                        key={m.id}
                        onClick={() => {
                          const newRatios = getVideoRatios(m.id);
                          const newResolutions = getVideoResolutions(m.id);
                          const newDurations = getVideoDurations(m.id);
                          let newRatio = promptParams.ratio || newRatios[0];
                          if (!newRatios.includes(newRatio)) newRatio = newRatios[0];
                          let newRes = promptParams.resolution || newResolutions[0];
                          if (!newResolutions.includes(newRes)) newRes = newResolutions[0];
                          let newDuration = promptParams.videoDurationId || newDurations[0];
                          if (!newDurations.includes(newDuration)) newDuration = newDurations[0];

                          updatePromptParams({ 
                            modelId: m.id,
                            ratio: newRatio,
                            resolution: newRes,
                            videoDurationId: newDuration
                          }); 
                          setVideoModelOpen(false); 
                        }}
                        className={cn(
          ...
          {data.error && !isGenerating && (
            <div className="bg-red-50 border border-red-100 rounded-xl p-3 flex items-start gap-2 text-red-600 mt-2">
              <span className="text-sm">⚠️</span>
              <div className="text-xs font-medium whitespace-pre-wrap flex-1">{data.error as string}</div>
            </div>
          )}
          ...
          <button 
            ...
            onClick={(e) => {
              if (isGenerating) {
                e.stopPropagation();
                if (data.onStop) (data.onStop as any)(id);
              } else {
                handleGenerate(e);
              }
            }}
          ...
          >
            ...
            <div className="relative flex items-center gap-2">
              ...
              <span className="tracking-widest uppercase">
                {isGenerating ? "停止生成..." : (mode === 'extract' ? "开始提取绘图素材" : "生成互动剧提示词")}
              </span>
            </div>
          </button>
        </div>
